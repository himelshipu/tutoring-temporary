...
class UserController extends Controller
{
           ...
           if ($request['project_title'] && count($request['project_title'])) {
            ...
            for ($i = 0; $i < count($request['project_title']); $i++) {
                $projects[] = [
                    ...
                    'technology_used' => $request->technology_used[$i],
                    'project_starts' => $request->project_starts[$i],
                    'project_ends' => $request->project_ends[$i],
    ...
    public function contentPartsJson($id)
    {
        ...
        foreach ($content->parts as $index => $part) {
            ...
            switch ($part['free']) {
                case '1':
                    $result[$index]['price'] = '<img src="/assets/default/images/svg/ulck.svg" style="width: 20px;height: auto;" title="Free">';
    ...
    public function ticketNotifications(Request $request)
    {
        ...
        $count = Notification::whereIn('recipent_type', ['user', 'userone'])
            ->where('recipent_list', $user->id)
            ->orWhere('recipent_type', 'all')
            ->where('mode', 'publish')
...
//    Github Redirect Function
    ...
    public function userProfileView($id)
    {
        ...
        $sells = 0;
        $lists = Content::where('user_id', $id)->withCount('sells', 'partsactive')->orderBy('id', 'DESC')->get();
        foreach ($lists as $list){
            $sells += $list->sells_count;
        }
        ...
        if ($user->vendor) {
            ...
            foreach ($quizzes as $quiz) {
                ...
                if ((isset($quiz->attempt) and count($quizResults) >= $quiz->attempt) or (!empty($quiz->result) and $quiz->result->status === 'pass')) {
                    $quiz->can_try = false;
                    $answers = $quizResults->toArray();
                    $quiz->answer_id = $answers[0]['id'];
                }
            }
        }
