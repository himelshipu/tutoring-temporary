...
function contentDuration($id){
    ...
    $meetings = \App\Models\MeetingDate::where('content_id', $id)->get()->toArray();
...
function channelRate($id){
    $Score  = 0;
    $Contents = \App\Models\ChannelVideo::where('chanel_id', $id)->pluck('content_id')->toArray();
    if(!is_array($Contents))
        return $Score;

    $Rates = \App\Models\ContentRate::whereIn('content_id',$Contents)->sum('rate');
    $RateCount = \App\Models\ContentRate::whereIn('content_id',$Contents)->count();

    if($Rates > 0 && $RateCount > 0)
        $Score = $Rates / $RateCount;

    return intval($Score);

}

function profileRate($user_id){
    $Score = 0;
    $Contents = \App\Models\Content::where('user_id', $user_id)->where('mode','publish')->pluck('id')->toArray();
    if(!is_array($Contents))
        return $Score;
...
function checkNotificationStatus($nId,$uId){
    $status = false;
    $count = \App\Models\NotificationStatus::where('user_id',$uId)->where('notification_id',$nId)->count();
    if($count > 0)
        $status = true;
...
if(file_exists(realpath(__DIR__.'../../../resources/views/web/plasma/functions.php'))){
    include(realpath(__DIR__.'../../../resources/views/web/plasma/functions.php'));
}
